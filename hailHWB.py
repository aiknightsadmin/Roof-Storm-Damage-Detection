...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 hailV3.py detect --weights=last --dataset=HailData --subset=predict
...
class HailDataset(utils.Dataset):
    def load_VIA(self, dataset_dir, subset, hc=False):
        ...
        self.add_class("HailDamage", 2, "blister")
        self.add_class("HailDamage", 3, "wind")
        if hc is True:
            for i in range(1,14):
                self.add_class("HailDamage", i, "{}".format(i))
            self.add_class("HailDamage", 14, "hailHWB")
...
def detect_and_color_splash(model, image_path=None, video_path=None, out_dir=''):
    ...
    class_names = ['BG', 'hail', 'blister','wind']
...
def detect(model, dataset_dir, subset):
    ...
    for image_id in dataset.image_ids:
        ...
        file_path = os.path.join(submit_dir, str(image_id)+".jpg")
